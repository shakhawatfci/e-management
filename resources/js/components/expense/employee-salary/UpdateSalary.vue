<template>
        <!-- Modal -->
    <div class="modal fade" id="UpdateEmployeeSalary" tabindex="-1" role="dialog" aria-labelledby="addContactModalTitle" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <form @submit.prevent="save()">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title">Update Employee Salary</h4>
                </div>
...
export default {
   ...
   mounted() {
  var _this = this;     
  EventBus.$on('edit-salary',function(salary){

  _this.salary = salary;
  _this.month = salary.month;
  $('#UpdateEmployeeSalary').modal('show');
  _this.getEmployee();
  });
 ...
 methods : {
    setMonth(){
     ...
     save()
     {
         ...
         axios.put(base_url+'employee-salary/'+this.salary.id,this.salary)
         .then(response => {
             if(response.data.status === 'success')
             {
              $('#UpdateEmployeeSalary').modal('hide');
